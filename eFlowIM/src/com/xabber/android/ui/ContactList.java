...
public class ContactList extends ManagedListActivity implements
		...
		OnItemClickListener, OnLongClickListener, Callback {
	...
	public static String IMAGE_ROOT_PATH = Environment
			.getExternalStorageDirectory().getPath() + "/chat/image/";
	
	public static final int imageMaxHW = 480;
	public static final int MSG_BUTTON_EXIT = 3001;
	public static final int MSG_BUTTON_RELOGIN = 3002;
	...
	public static String prefix = "http://121.199.3.19:8080";
...
//		registerForContextMenu(listView);
		...
		contactListAdapter.giveMeHandler(mHandler);
...
//		showDialog(DIALOG_OPEN_WITH_ACCOUNT_ID);
	...
	public void onCreateContextMenu(ContextMenu menu, View view,
		...
		if (view == getListView()) {
			...
			// Account panel
			actionWithAccount = (String) accountToggleAdapter.getItemForView(view);
			actionWithGroup = null;
			actionWithUser = null;
		}
		// Group or account
...
//					.setMessage(getString(R.string.start_at_boot_suggest)).create();
...
//				Log.d("", " receive file" + request );
				if (shouldAccept(request)) {
					// Accept it
					IncomingFileTransfer transfer = request.accept();
					try {
						System.out.println("接收文件："+request.getFileName() + " . "+ request.getMimeType());
						File file = null;
						
						if ( request.getMimeType().startsWith("audio") ){
							file = new File(RECORD_ROOT_PATH + request.getFileName());
						}else if ( request.getMimeType().startsWith("image") ) {
							file = new File(IMAGE_ROOT_PATH + request.getFileName());
						}else {
							file = new File(FILE_ROOT_PATH + request.getFileName());
						}
	...
	public void onItemClick(AdapterView<?> parent, View view, int position,	long id) {
		Iterator<String> acdeString = AccountManager.getInstance().getAccounts().iterator();
		AccountItem accountItem = AccountManager.getInstance().getAccount(acdeString.next());
		boolean online;
		if (accountItem == null)
			online = false;
		else
	...
	public boolean handleMessage(Message msg) {
		switch (msg.what) {
		case HANDLER_MSG_1:
			Log.i("", "size: "+ AccountManager.getInstance().getAccounts().size());
		...
		case MSG_BUTTON_RELOGIN:
			Iterator<String> acdeString = AccountManager.getInstance().getAccounts().iterator();
			AccountItem accountItem = AccountManager.getInstance().getAccount(acdeString.next());
			String account = accountItem.getAccount();
			
			AccountManager.getInstance().removeAccountWithoutCallback(account);
			Application.getInstance().requestToClose();
			unregisterListeners();
			new Handler().postDelayed(new Runnable() {
				@Override
